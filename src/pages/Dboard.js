...
function Dboard() {
  const [showFinalizeView, setShowFinalizeView] = useState(false);
...
const handleButtonPress = async () => {
  ...
  try {
    ...
    alert("Price has been loaded by machine learning model");
  } catch (error) {
  ...
  {showFinalizeView && (
      <div>
        <div className="text-white text-2xl mb-6">
          This is the Finalized View after clicking "Finalize" button.
        </div>
        <div className="text-white text-xl mb-4">
          <p>Thank you for finalizing the trip!</p>
          <p>You can view the optimized payment distribution below:</p>
        </div>
        <div className="text-white text-lg mb-4">
          {finalGraphEdges.map((edge, index) => (
            <p key={index}>
              {edge[0]} owes ${edge[2]} to {edge[1]}
            </p>
          ))}
        </div>
      </div>
