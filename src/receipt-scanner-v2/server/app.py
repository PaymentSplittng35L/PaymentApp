...
Condenses preprocessing and OCR into one script
-just input path to image
...
def main():
    ...
    argc = len(sys.argv)
    if (argc != 2):
        sys.stderr.write("Error: invalid number of parameters, should be one")
        exit(1)
    img = cv2.imread(sys.argv[1])
    if img is None:
        sys.stderr.write("Error: invalid parameter, image could not be found")
        exit(1)
    ...
    cv2.imwrite("temp/final.jpg", padded)

    #OCR
    data = get_numbers()
    img = cv2.imread("temp/final.jpg")
    size = len(data['text'])
    max_val = 0.0
    max_ind = 0
    ...
    cv2.imwrite("results/final.jpg", img)
    cv2.imshow("img", img)
    cv2.waitKey(0)
